package scheduler.aser.reex;

//import edu.illinois.imunit.internal.parsing.*;
//import edu.tamu.aser.internaljuc.*;
...
import edu.illinois.imunit.internal.parsing.*;
import scheduler.aser.internaljuc.*;
import scheduler.aser.listeners.Listeners;
import scheduler.aser.runtime.RVRunTime;
import scheduler.aser.scheduling.MCRProperties;
import scheduler.aser.scheduling.events.*;
import scheduler.aser.scheduling.filtering.DefaultFilter;
import scheduler.aser.scheduling.filtering.SchedulingFilter;
import scheduler.aser.scheduling.strategy.ChoiceType;
import scheduler.aser.scheduling.strategy.DefaultStrategy;
import scheduler.aser.scheduling.strategy.SchedulingStrategy;
...
public class Scheduler {
    ...
    private static Map<Thread, ThreadInfo> liveThreadInfos;   //程序中所有的线程
    private static SortedSet<ThreadInfo> pausedThreadInfos;   //由于下一步即将运行到某种插桩点之前而人为暂停的线程
    private static Set<ThreadInfo> blockedThreadInfos;        //由于程序运行而block掉的线程
                ...
                while (true) {  
                    ...
                    try {
                        if (!liveThreadInfos.isEmpty()) {
                            ...
                            else if(timeout && !pausedThreadInfos.isEmpty())//JEFF   处理超时
                            {
    ...
    private static void beforeEvent(EventDesc eventDesc, boolean pause) {
        ...
        try {
            ...
            if(currentThreadInfo!=null)
            {
                ...
                if (pause) {  //pause参数用来判断是否需要在插桩点运行之前阻塞线程
                    pausedThreadInfos.add(currentThreadInfo);   //将当前线程ThreadInfo加入了pausedThreadInfo中
                }
            }
        } finally {
            if (pause) {
...
//                    System.err.println("---sema accquired by: "+currentThreadInfo.toString());
                    currentThreadInfo.getPausingSemaphore().acquire();  //使调用beforeEvent()的线程阻塞
//                    System.err.println("***sema accquired by: "+currentThreadInfo.toString());
                }
            }
        } catch (InterruptedException e) {
