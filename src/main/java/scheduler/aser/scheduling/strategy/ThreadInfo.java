package scheduler.aser.scheduling.strategy;
...
import scheduler.aser.internaljuc.Reex_Semaphore;
...
public class ThreadInfo implements Comparable<ThreadInfo> {
    ...
    private final Thread thread;     //线程本身
    private final Reex_Semaphore pausingSemaphore;    //暂停信号量
    private IdentityHashMap<Object, Integer> lockSet;  //锁集
    private boolean parkPermitAvailable;   //可暂停
    private EventDesc eventDesc;        //事件类型
    private LocationDesc locationDesc;   //位置
    private int runCount;           //线程当前执行行数
